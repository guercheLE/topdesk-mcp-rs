...
TOPdesk MCP (Model Context Protocol) server, generated by [mcpify](https://github.com/guercheLE/mcpify) — despite its default version's name, this isn't limited to TOPdesk's "General" API: the `versions` command lists 18 separate specs it can switch between, spanning Incident, Change Management, Asset Management, Reservations, Access Roles, and a dozen more of TOPdesk's published APIs (see [`docs/SCHEMA_VERSIONS.md`](docs/SCHEMA_VERSIONS.md) for the full list).
...
Exposes exactly 3 tools — `search`, `get`, `call` — backed by an embedded semantic database (`mcp_store.db`), so an LLM never needs the full API surface in context. It also exposes MCP **prompts** — guided, multi-step playbooks over those same 3 tools for common TOPdesk workflows; see [Guided workflow prompts](#guided-workflow-prompts) below.
...
Beyond the 3 generic tools, this server exposes MCP **prompts** — a `topdesk-menu` entry point plus one guided playbook per TOPdesk domain. Each playbook sequences `search`/`get`/`call` into a numbered set of steps for that domain's workflow (creating and escalating an incident, routing a change through approval, booking a reservation, and so on), including gates at points where TOPdesk's real semantics require a human decision (e.g. an approval must have actually happened, not be self-granted) and forks where the workflow genuinely branches (e.g. standard vs. extensive vs. emergency change).

Fetch `topdesk-menu` first — any MCP client that supports `prompts/list`/`prompts/get` can call it with no arguments to see the full directory, including exactly which `api_version` catalog each playbook needs:
